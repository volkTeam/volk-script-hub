    ...
    onSnapshot(collection(db, "users"), (snapshot) => {
        ...
        snapshot.docs.forEach(d => {
            ...
            if (data.username) {
                ...
                userRolesCache[lower] = data.role || 'user';
                userBiosCache[lower] = data.bio || '';
            }
        });
...
window.handleUpload = async function(event) {
    ...
    const hasKey = document.getElementById('scriptHasKey').checked;
...
/* ---------------------------------------------------------
   PROFILE TAB MANAGEMENT (BIO & MY SCRIPTS EDIT)
   --------------------------------------------------------- */
    ...
    if (currentUser.role === 'guest') {
        container.innerHTML = `
            <div class="warning-box">
                <p>You are viewing as Guest. Please sign in to access your profile.</p>
                <button class="action-btn primary" onclick="openAuthModal()" style="margin-top:10px;">Sign In</button>
            </div>
        `;
        return;
    }

    const lowerName = currentUser.username.toLowerCase();
    const currentBio = userBiosCache[lowerName] || '';
    const formattedUser = formatUserRoleBadge(currentUser.username, currentUser.role);

    // Фильтруем скрипты текущего пользователя
    const myScripts = allScripts.filter(s => (s.author || '').toLowerCase() === lowerName);

    let myScriptsHtml = myScripts.map(s => `
        <div class="script-card" style="margin-bottom:12px; cursor:default;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <h3>${s.isTrojan ? '[DETECTED TROJAN] ' : ''}${s.title} ${s.hasKey ? '<span style="color:#f59e0b; font-size:12px;">🔑 (Key Required ✔️)</span>' : ''}</h3>
                <div>
                    <button class="action-btn primary" onclick="editMyScript('${s.id}')" style="padding:4px 8px; font-size:12px;">Edit</button>
                    <button class="action-btn danger" onclick="deleteMyScript('${s.id}')" style="padding:4px 8px; font-size:12px;">Delete</button>
                </div>
            </div>
            <p style="color:var(--text-muted); font-size:12px; margin-top:4px;">Game: ${s.game}</p>
            <p style="font-size:13px; margin:6px 0;">${s.desc}</p>
        </div>
    `).join('');

    if (myScripts.length === 0) {
        myScriptsHtml = '<p style="color:var(--text-muted); font-size:13px;">You have not published any scripts yet.</p>';
    }

    container.innerHTML = `
        <div class="form-card" style="margin-bottom:20px;">
            <h3>User Info</h3>
            <p style="font-size:18px; margin:10px 0;"><strong>Username:</strong> ${formattedUser}</p>
            
            <div class="form-group" style="margin-top:15px;">
                <label>Profile Bio (Description)</label>
                <textarea id="profileBioInput" rows="3" placeholder="Tell the community about yourself...">${currentBio}</textarea>
            </div>
            <button class="action-btn primary" onclick="saveProfileBio()">Save Bio</button>
        </div>

        <div class="form-card">
            <h3>My Scripts (${myScripts.length})</h3>
            <p style="color:var(--text-muted); font-size:12px; margin-bottom:15px;">Manage and update your published scripts below:</p>
            <div id="myScriptsList">${myScriptsHtml}</div>
        </div>
    `;
}

window.saveProfileBio = async function() {
    const bioText = document.getElementById('profileBioInput').value.trim();
    const userRef = doc(db, "users", currentUser.username.toLowerCase());

    try {
        await setDoc(userRef, { bio: bioText }, { merge: true });
        alert("Profile bio saved successfully!");
    } catch (e) {
        alert("Failed to save bio: " + e.message);
    }
};

window.editMyScript = async function(scriptId) {
    const script = allScripts.find(s => s.id === scriptId);
    if (!script) return;

    const newTitle = prompt("Update Script Title:", script.title);
    if (newTitle === null) return;

    const newGame = prompt("Update Game Name:", script.game);
    if (newGame === null) return;

    const newDesc = prompt("Update Description:", script.desc);
    if (newDesc === null) return;

    const newCode = prompt("Update Lua Code:", script.code);
    if (newCode === null) return;

    const hasKeyConfirm = confirm("Does this script require a Key System?");
...
window.deleteMyScript = async function(scriptId) {
...
window.devAssignModerator = async function() {
    ...
    await setDoc(userRef, { username: userInput, role: 'admin' }, { merge: true });

    alert(`User '${userInput}' was granted ADMIN status! They now have full admin panel access.`);
};
...
function renderScripts(filterText = '') {
    ...
    filtered.forEach(script => {
        ...
        const keyBadge = script.hasKey ? `<span style="color:#f59e0b; font-size:12px; margin-left:6px;"><i class="fa-solid fa-key"></i> Key Required ✔️</span>` : '';

        card.innerHTML = `
            <div class="game-name"><i class="fa-solid fa-gamepad"></i> ${script.game} ${keyBadge}</div>
            <h3>${titleText}</h3>
...
window.viewScriptDetail = function(scriptId) {
    ...
    const formattedAuthor = formatUserRoleBadge(script.author, script.authorRole);
    const keyStatusText = script.hasKey 
        ? `<span style="color:#f59e0b; font-weight:bold; font-size:13px; margin-left:10px;"><i class="fa-solid fa-key"></i> Key System Required ✔️</span>` 
        : `<span style="color:var(--text-muted); font-size:13px; margin-left:10px;">No Key Required</span>`;
    ...
    container.innerHTML = `
        <div class="script-detail-box">
            <span style="color: var(--accent); font-size: 14px;"><i class="fa-solid fa-gamepad"></i> ${script.game} ${keyStatusText}</span>
            <h2>${displayTitle}</h2>
...
function renderTrending() {
    ...
    sorted.forEach(script => {
        ...
        card.onclick = () => window.viewScriptDetail(script.id);
        const formattedAuthor = formatUserRoleBadge(script.author, script.authorRole);
        const keyBadge = script.hasKey ? `<span style="color:#f59e0b; font-size:12px; margin-left:6px;">🔑 Key Required</span>` : '';

        card.innerHTML = `
            <div class="game-name"><i class="fa-solid fa-fire" style="color: #f97316;"></i> ${script.game} ${keyBadge}</div>
