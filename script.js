...
let allScripts = [];
let userRolesCache = {}; // Живой кэш ролей всех пользователей
...
// Форматирование никнейма с цветным статусом роли (берет актуальную роль из базы)
...
function monitorSession(username) {
    ...
    sessionUnsubscribe = onSnapshot(userDocRef, (snapshot) => {
        if (snapshot.exists()) {
            ...
            // Динамическое обновление роли при выдаче Админки в реальном времени
...
window.adminBanUser = async function() {
    ...
    await setDoc(targetRef, { isBanned: true }, { 
    merge: true });
...
// ВЫДАЧА АДМИНКИ ПО НИКУ (Запись прямо в Firestore документе)
