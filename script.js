...
// === ПУБЛИКАЦИЯ СКРИПТА С ТЕГАМИ И КУЛДАУНОМ ===
async function publishScript() {
  ...
  if (!title || !game || !code) return alert('Заполните поля!');

  // Кулдаун (30 дней для обычных, 7 дней для Verified)
  if (currentUser.role !== 'admin') {
...
// === ВЗАИМОДЕЙСТВИЯ (ЛАЙКИ, ИЗБРАННОЕ, СЧЁТЧИКИ) ===
async function copyCode(id, code) {
  navigator.clipboard.writeText(decodeURIComponent(code));
  alert('Код скопирован!');
  const { data } = await supabase.from('scripts').select('views_copies').eq('id', id).single();
  if (data) await supabase.from('scripts').update({ views_copies: (data.views_copies || 0) + 1 }).eq('id', id);
}

async function likeScript(id) {
  const { data } = await supabase.from('scripts').select('likes').eq('id', id).single();
  if (data) {
    await supabase.from('scripts').update({ likes: (data.likes || 0) + 1 }).eq('id', id);
    loadScripts();
  }
}
...
async function reportScript(id) {
  ...
  if (reason) {
    await supabase.from('reports').insert([{ script_id: id, reason }]);
    alert('🚩 Жалоба отправлена модераторам!');
  }
}

// === АДМИН-ФУНКЦИИ И ПАНЕЛЬ МОДЕРАЦИИ ===
...
async function toggleVerified(username, give) {
  ...
  await supabase.from('scripts').update({ author_role: role }).eq('author_username', username);
  loadScripts();
}
