...
/* ---------------------------------------------------------
   PC ADAPTABILITY & SCALING SYSTEM
   --------------------------------------------------------- */
function initPCAdaptability() {
    if (!document.getElementById('pcAdaptabilityStyles')) {
        const style = document.createElement('style');
        style.id = 'pcAdaptabilityStyles';
        style.innerHTML = `
...
/* ---------------------------------------------------------
   COSMIC SATURN 3D PARTICLE LOADER (MARKETING ANIMATION)
   --------------------------------------------------------- */
function showSaturnLoader() {
    let saturnOverlay = document.getElementById('saturnLoaderOverlay');
    if (!saturnOverlay) {
        saturnOverlay = document.createElement('div');
        saturnOverlay.id = 'saturnLoaderOverlay';
        saturnOverlay.style.cssText = `
            position: fixed;
            top: 0;
            left: 0;
            width: 100vw;
        ...
        saturnOverlay.innerHTML = `
            <div style="color: #60a5fa; font-family: 'Segoe UI', Roboto, sans-serif; font-size: 22px; font-weight: 800; letter-spacing: 8px; text-shadow: 0 0 20px #3b82f6; margin-bottom: 25px; animation: saturnPulse 1.5s infinite alternate;">LOADING...</div>
            <canvas id="saturnCanvas" width="340" height="340"></canvas>
        `;
        document.body.appendChild(saturnOverlay);
    } else {
        saturnOverlay.style.display = 'flex';
        saturnOverlay.style.opacity = '1';
    }
    ...
    for (let i = 0; i < numParticles; i++) {
        const radius = 65 + Math.random() * 75; // Ring radius range
        const angle = Math.random() * Math.PI * 2;
        const speed = (0.008 + Math.random() * 0.012) * (Math.random() < 0.5 ? 1 : 1);
        const size = Math.random() * 2 + 0.8;
        const hue = 180 + Math.random() * 140; // Cyan to Purple/Pink spectrum
        particles.push({ radius, angle, speed, size, hue, yTilt: 0.32 });
    }
    ...
    function renderSaturn() {
        ...
        rotationGlobal += 0.015;

        // Draw Saturn Center Core (Dark Cosmic Sphere)
        const planetGradient = ctx.createRadialGradient(cx - 10, cy - 10, 5, cx, cy, 38);
        planetGradient.addColorStop(0, '#3b82f6');
        planetGradient.addColorStop(0.4, '#1e1b4b');
        planetGradient.addColorStop(1, '#050515');

        // Back Ring Particles
        particles.forEach(p => {
            p.angle += p.speed;
            const x = cx + Math.cos(p.angle) * p.radius;
            const y = cy + Math.sin(p.angle) * p.radius * p.yTilt;

            // Render back particles behind planet
        ...
        // Front Ring Particles
        particles.forEach(p => {
            const x = cx + Math.cos(p.angle) * p.radius;
            const y = cy + Math.sin(p.angle) * p.radius * p.yTilt;

            // Render front particles in front of planet
            if (Math.sin(p.angle) >= 0) {
                ctx.beginPath();
        ...
        animId = requestAnimationFrame(renderSaturn);
    }

    renderSaturn();

    // Fade out precisely after 5 seconds
    setTimeout(() => {
        saturnOverlay.style.opacity = '0';
        setTimeout(() => {
...
window.passBotVerify = function() {
    ...
    if (overlay) {
        ...
        setTimeout(() => {
            ...
            // Trigger 5-Second Saturn Animation after Bot Check
            showSaturnLoader();
        }, 300);
    }
};
...
window.handleUpload = async function(event) {
    ...
    const keySystem = document.getElementById('scriptKeySystem').checked;
...
/* ---------------------------------------------------------
   YOUTUBER COPYRIGHT STRIKE SYSTEM
   --------------------------------------------------------- */
...
window.viewScriptDetail = function(scriptId) {
    ...
    // ONE ROW COPY LAYOUT & CHUNKING LOGIC
    let codeBlocksHtml = '';
    if (script.isTrojan || script.isCopyrightBlocked) {
        ...
        if (script.code.length > chunkSize) {
            codeBlocksHtml += `<p style="color: var(--warning); font-size: 12px;">Code split into equal parts for smooth copying:</p>`;
            for (let i = 0; i < script.code.length; i += chunkSize) {
                ...
                codeBlocksHtml += `
                    <div class="one-line-copy-row" style="display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 10px; background: rgba(255,255,255,0.03); padding: 8px 12px; border-radius: 8px; border: 1px solid var(--border-color, #333);">
                        <span style="font-size:13px; font-weight:bold; white-space:nowrap; color:var(--accent, #22c55e);">Part ${Math.floor(i/chunkSize) + 1}</span>
                        <div class="code-block" style="margin: 0; flex-grow: 1; overflow-x: auto; max-height: 50px;">${chunk}</div>
                        <button class="action-btn primary" style="white-space: nowrap; margin: 0; flex-shrink: 0;" onclick="copyScriptPart('${safeChunk}')">
            ...
            codeBlocksHtml = `
                <div class="one-line-copy-row" style="display: flex; align-items: center; justify-content: space-between; gap: 10px; margin: 15px 0;">
                    <div class="code-block" style="margin: 0; flex-grow: 1;">${script.code}</div>
                    <button class="action-btn primary" style="white-space: nowrap; margin: 0; flex-shrink: 0; align-self: stretch; display: flex; align-items: center; justify-content: center;" onclick="copyScriptPart('${safeCode}')">
