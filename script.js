...
function renderScripts(filterText = '') {
    ...
    filtered.forEach(script => {
        ...
        let timestampAttr = (typeof script.id === 'number' && script.id > 1000000000000) ? script.id : '';
        ...
        card.innerHTML = `
            ...
            <div class="script-meta">
                ...
                <span><i class="fa-solid fa-clock"></i> <span class="script-date" data-created-time="${timestampAttr}">${script.time}</span></span>
            </div>
        `;
...
/* ---------------------------------------------------------
   REAL-TIME SCRIPT TIMER (SEKUNDOMER)
   --------------------------------------------------------- */
function formatPassedTime(totalSeconds) {
    if (totalSeconds < 5) return 'только что';
    if (totalSeconds < 60) return `${totalSeconds} сек. назад`;
    
    const minutes = Math.floor(totalSeconds / 60);
    const seconds = totalSeconds % 60;
    if (minutes < 60) return `${minutes} мин. ${seconds} сек. назад`;
    
    const hours = Math.floor(minutes / 60);
    const remMinutes = minutes % 60;
    if (hours < 24) return `${hours} ч. ${remMinutes} мин. назад`;
